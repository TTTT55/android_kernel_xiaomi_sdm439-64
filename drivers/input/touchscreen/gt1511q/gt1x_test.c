...
#define GT1X_ITO_TEST                "self_test"
#define GT1X_SELF_TEST               "tp_selftest"

static struct proc_dir_entry *gt1x_ito_test_proc;
static struct proc_dir_entry *gt1x_android_touch_proc;
static struct proc_dir_entry *gt1x_selftest_proc;

int gt1x_test_result = RESULT_INVALID;
...
FILE *fopen(const char *path, const char *mode)
{
	...
	if (!strcmp(mode, "a+")) {
		/*if(file_pos == 0) {
			...
			filp = file_open_name(vts_name, O_RDWR | O_CREAT, 0666);
		}
		if (!IS_ERR(filp)) {
		...
		vts_name = getname_kernel(path);
		filp = file_open_name(vts_name, O_RDONLY, 0666);
...
int check_version(unsigned char *ini_version)
{
	...
	snprintf((char *)tmp, 15, "%d.%d.%d_%d.%d", (gt1x_version.mask_id >> 16) & 0x0FF, (gt1x_version.mask_id >> 8) & 0x0FF, (gt1x_version.mask_id & 0xFF),
...
int disable_hopping(unsigned char *config, int len, int flag)
{
	...
	do {
		...
		if (!memcmp(&config[1], &tmp_config[1], len - 130)) {
			break;
		}
	} while (++i < 1);

	if (i >= 1) {
		GTP_ERROR("ITO test has problem!");
	}
...
/*int auto_find_ini(char *findDir, char *format, char *inipath)
{
	...
	sprintf((char *)tmp, "/vendor/etc/test_sensor_%d.ini", gt1x_version.sensor_id);
...
int auto_find_ini(char *findDir, char *format, char *inipath)
{
	...
	snprintf((char *)tmp, 35, "/system/etc/test_sensor_%d.ini", gt1x_version.sensor_id);

	vts_name = getname_kernel(tmp);
	pfile = file_open_name(vts_name, O_RDONLY, 0);

	if (IS_ERR(pfile)) {
		GTP_ERROR("error occured while opening file.");
		return -EIO;
	}
...
static int gtp_sysfs_openshort_show(struct seq_file *file, void *data)
{
	...
	if (ito_result == _TEST_PASS) {
		len += snprintf((char *)&ito_buf[len], 30, "0P-1P-2P-3P-4P\r\n");
	} else {
		//len += sprintf((char *)&buff[len], "NG\r\n");
		GTP_ERROR("test error code 0x%x\n", ito_result);
		if (ito_result & _I2C_ERR) {
			len += snprintf((char *)&ito_buf[len], 3, "0F");
		} else if (!(ito_result & _I2C_ERR)) {
			len += snprintf((char *)&ito_buf[len], 3, "0P");
		}

		if (ito_result & _BEYOND_MAX_LIMIT) {
			len += snprintf((char *)&ito_buf[len], 4, "-1F");

		} else if (!(ito_result & _BEYOND_MAX_LIMIT)) {
			len += snprintf((char *)&ito_buf[len], 4, "-1P");

		}

		if (ito_result & _BEYOND_MIN_LIMIT) {
			len += snprintf((char *)&ito_buf[len], 4, "-2F");
		} else if (!(ito_result & _BEYOND_MIN_LIMIT)) {
			len += snprintf((char *)&ito_buf[len], 4, "-2P");
		}

		if (ito_result & _BEYOND_ACCORD_LIMIT) {
			len += snprintf((char *)&ito_buf[len], 4, "-3F");
		} else if (!(ito_result & _BEYOND_ACCORD_LIMIT)) {
			len += snprintf((char *)&ito_buf[len], 4, "-3P");
		}

		if (ito_result & _GT_SHORT) {
			len += snprintf((char *)&ito_buf[len], 7, "-4F\t\n\n");
			//len += sprintf((char *)&buff[len], "short circuit\r\n");
			for (i = 0; i < result[0]; i++) {
				if (result[1 + i * 4] & 0x80) {
					len += snprintf((char *)&ito_buf[len], 10, "drv%d & ", result[1 + i * 4] & 0x7F);
				} else if (result[1 + i * 4] == (sys.max_driver_num + 1)) {
					len += snprintf((char *)&ito_buf[len], 15, "GND\\VDD & ");
				} else {
					len += snprintf((char *)&ito_buf[len], 10, "sen%d & ", result[1 + i * 4] & 0x7F);
				}

				if  (result[2 + i * 4] & 0x80) {
					len += snprintf ((char *)&ito_buf[len], 8, "drv%d", result[2 + i * 4] & 0x7F);
				} else if (result[2 + i * 4] == (sys.max_driver_num + 1)) {
					len += snprintf ((char *)&ito_buf[len], 15, "GND\\VDD");
				} else {
					len += snprintf ((char *)&ito_buf[len], 10, "sen%d", result[2 + i * 4] & 0x7F);
				}

				len += snprintf ((char *)&ito_buf[len], 5, "(%dk)\r\n", ((result[3 + i * 4] << 8) + result[4 + i * 4]) / 10);
			}
		} else if (!(ito_result &  _GT_SHORT)) {
			len += snprintf ((char *)&ito_buf[len], 7, "-4P\t\n");
		}
	}
	...
	size_t count, loff_t *pos)
{
	...
	if (fp == NULL) {
		...
		GTP_ERROR("open /sdcard/test.csv fail");
	...
	for (i = 0; i < 100; i++) {
		...
		bytes += snprintf(&buff[bytes], 100, "%d,", i);
...
static ssize_t gt1x_tp_selftest_read(struct file *file, char __user *buf,
				      size_t count, loff_t *pos)
{
	char tmp[5];
	int cnt;
	...
	cnt = snprintf(tmp, sizeof(gt1x_test_result), "%d\n", gt1x_test_result);
...
#define SELFTEST_RESULT_BUF 300
static ssize_t gt1x_tp_selftest_write(struct file *file, const char __user *buf,
				       size_t count, loff_t *pos)
{
	char tmp[6];
	unsigned char *selftest_result = NULL;
	u32 retval = 0;
	int r = -1;
	...
	if (copy_from_user(tmp, buf, count)) {
		...
		gt1x_test_result = RESULT_INVALID;
		goto out;
	}
	...
	if  (r < 0) {
		...
		selftest_result = (unsigned char *)malloc(SELFTEST_RESULT_BUF);
		if (selftest_result == NULL) {
			GTP_ERROR("selftest_result malloc fail!!!");
			return INVALID;
		} else {
			if (!strncmp(tmp, "short", 5)) {
				retval = open_short_test(selftest_result);
				retval = retval & _GT_SHORT;
			} else if (!strncmp(tmp, "open", 4)) {
				retval = open_short_test(selftest_result);
				retval = (retval & _BEYOND_MAX_LIMIT) || (retval & _BEYOND_MIN_LIMIT) || (retval & _BEYOND_ACCORD_LIMIT);
			} else if (!strncmp(tmp, "i2c", 3)) {
				retval = open_short_test(selftest_result);
				retval = retval & _I2C_ERR;
			} else {
				retval = -EFAULT;
				gt1x_test_result = RESULT_INVALID;
				GTP_ERROR("Invalid teset case!\n");
				goto out;
			}
			if (retval == FAIL) {
				gt1x_test_result = RESULT_NG;
				GTP_DEBUG("I2C test failed!\n");
			} else {
				gt1x_test_result = RESULT_PASS;
				GTP_DEBUG("I2C test pass!\n");
			}
		}
	}
...
static const struct file_operations tp_selftest_ops = {
	.read		= gt1x_tp_selftest_read,
	.write		= gt1x_tp_selftest_write,
};
...
s32 gtp_test_sysfs_init(void)
{
	...
	gt1x_selftest_proc = proc_create(GT1X_SELF_TEST, 0664, NULL, &tp_selftest_ops);

	gt1x_android_touch_proc = proc_mkdir(GT1X_ANDROID_TOUCH, NULL);
	gt1x_ito_test_proc = proc_create(GT1X_ITO_TEST, 0664,
					      gt1x_android_touch_proc, &ito_test_ops);
